...
That produced a real bug on the first redesign deploy. A browser that had
the site open shortly before fetched the **new** `index.html` but reused
the **old** `app.js`. The old script referenced an element the redesign had
removed, threw a `TypeError`, and left the page half-drawn: the score
appeared, the breakdown never did, and nothing said anything was wrong.

The mismatch, not the staleness, is what breaks a page. So the fix keeps
the two in lockstep: the stylesheet and entry script are requested as
`style.css?v=2` and `app.js?v=2`. New HTML therefore asks for a *new URL*
for its script and always gets the matching copy. The remaining ten-minute
window now serves old HTML with old JavaScript, which is internally
consistent and works fine.

Two supporting changes:

- **`model.json` and `phrases.json` are fetched with `cache: "no-cache"`,**
  forcing revalidation. These change on every retrain, and scoring someone
  with last week's weights while claiming to use this week's is the worst
  failure available here. It is not `no-store`: the browser still caches
  and usually gets a small `304`.
- **Rendering is wrapped in `try/catch`,** so any future mismatch asks the
  visitor to reload instead of silently half-rendering.

The version number is manual, which is the weak point. The industry fix is
content-hashed filenames (`app.8f3a2b.js`) generated by a build step; this
project deliberately has no build step, so the trade is a number to
remember on shape changes.

### 3.4 Logistic regression rather than something more powerful
...
### 3.6 Labeling tool as a local web page
...
### 3.7 Evidence checkboxes: the label-quality problem
...
### 3.8 Honest evaluation, built into the tooling
...
1. **The labels are educated guesses.** No ground truth exists. See 3.7.
...
6. **No interaction effects.** See 3.4.
...
**Q: You have no build step. How do you handle cache invalidation?**
- GitHub Pages sets `max-age=600`, so browsers hold files for ten minutes.
- This bit me for real: a browser fetched new HTML but reused the old
  script, which referenced an element the redesign had deleted. It threw and
  left the page half-drawn, with no error shown to the user.
- The insight is that **staleness is fine; a mismatch is not**. Old HTML with
  old JavaScript works correctly. So I version the asset URLs
  (`app.js?v=2`), which makes new HTML request a new URL and guarantees the
  pair arrives together.
- The data files additionally use `cache: "no-cache"` to force
  revalidation, since they change on every retrain.
- The weakness is that the version number is manual. The industry fix is
  content-hashed filenames from a build step, which I traded away to keep the
  project buildless.
